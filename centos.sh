#!/usr/bin/env bash
# CentOS Stream 9 / RHEL 9 Optimized Installer for Pelican
# This script is adapted from ubuntu.sh for CentOS/RHEL systems
# Safe Bash settings
set -euo pipefail
IFS=$'\n\t'
...
# Official Docker installation for CentOS/RHEL
install_docker() {
        ...
        (dnf install -y dnf-plugins-core >/dev/null 2>&1) & show_spinner $!
...
# Check if CentOS/RHEL
if [ -f /etc/redhat-release ]; then
        DISTRO=$(cat /etc/redhat-release)
        info "Detected: $DISTRO"
else
        warn "This script is designed for CentOS/RHEL. Proceeding anyway."
fi
...
if ! $SKIP_WELCOME; then
    clear
    echo "Welcome to Pelican Pouch (CentOS/RHEL Edition)"; sleep 1
...
#-------------- Base Packages --------------#
info "Installing EPEL and base tools"
(dnf install -y epel-release >/dev/null 2>&1) & show_spinner $!
(dnf install -y ca-certificates curl wget git unzip tar >/dev/null 2>&1) & show_spinner $!
...
#-------------- PHP Setup --------------#
info "Setting up PHP 8.4 from Remi repository"
...
PHP_PACKAGES="php php-fpm php-gd php-mysqlnd php-mbstring php-bcmath php-xml php-curl php-zip php-intl php-sqlite3"
info "Installing PHP 8.4 & extensions"
(dnf install -y $PHP_PACKAGES >/dev/null 2>&1) & show_spinner $!
...
# Configure PHP-FPM to use nginx user
...
if command -v getenforce >/dev/null 2>&1 && [ "$(getenforce)" != "Disabled" ]; then
        info "Configuring SELinux permissions"
        (setsebool -P httpd_can_network_connect 1 >/dev/null 2>&1) & show_spinner $!
        (chcon -R -t httpd_sys_rw_content_t /var/www/pelican/storage >/dev/null 2>&1) & show_spinner $!
        (chcon -R -t httpd_sys_rw_content_t /var/www/pelican/bootstrap/cache >/dev/null 2>&1) & show_spinner $!
fi
