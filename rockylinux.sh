#!/usr/bin/env bash
# Rocky Linux 9 Optimized Installer for Pelican
# This script is adapted from ubuntu.sh for Rocky Linux 9
# Safe Bash settings
set -euo pipefail
IFS=$'\n\t'
...
# Official Docker installation for Rocky Linux
install_docker() {
...
if [ -f /etc/rocky-release ]; then
        ROCKY_VERSION=$(rpm -q --queryformat '%{VERSION}' rocky-release)
        if [ "$ROCKY_VERSION" != "9" ]; then
                warn "This script was tailored for Rocky Linux 9. Detected: $ROCKY_VERSION. Proceeding anyway."
        fi
else
        error_exit "This script is for Rocky Linux only."
fi
...
if ! $SKIP_WELCOME; then
    clear
    echo "Welcome to Pelican Pouch (Rocky Linux 9 Edition)"; sleep 1
...
(dnf install -y ca-certificates curl wget git unzip tar gnupg2 >/dev/null 2>&1) & show_spinner $!
...
(dnf install -y php php-fpm php-gd php-mysqlnd php-mbstring php-bcmath php-xml php-curl php-zip php-intl php-sqlite3 php-json php-opcache >/dev/null 2>&1) & show_spinner $!
